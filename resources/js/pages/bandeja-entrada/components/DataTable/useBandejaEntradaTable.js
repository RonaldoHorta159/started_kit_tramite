...
export default function useBandejaEntradaTable(
    dataRef,
    filterRef,
    actions = {},
) {
    ...
    const table = useVueTable({
        ...
        onColumnVisibilityChange: (v) => (columnVisibility.value = typeof v === 'function' ? v(columnVisibility.value) : v),
        onRowSelectionChange: (v) => (rowSelection.value = typeof v === 'function' ? v(rowSelection.value) : v),
    ...
    watch(dataRef, (nv) => {
        ...
        table.setOptions((opts) => ({ ...opts, data: rowsRef.value, pageCount: lastPageRef.value }));
    }, { deep: true });
    ...
    const estadoSet = computed(() => new Set(table.getColumn('estado')?.getFilterValue() ?? []));
    function toggleEstado(val, checked) {
        const set = new Set(estadoSet.value);
        checked ? set.add(val) : set.delete(val);
        table.getColumn('estado')?.setFilterValue(set.size ? Array.from(set) : undefined);
    }
